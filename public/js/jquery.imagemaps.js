    ...
    $.fn.imageMaps = function(options) {
        ...
        this.each(function(i, item){
            ...
            $(options.addBtn+':eq('+ i +')').unbind('click').on('click', function(){                
                ...
                let oDiv   = $(`<div id="imagemaps-rect-${i}-${index}" style="width:${(coords[2]-coords[0]) || options.rectWidth}px;height:${(coords[3]-coords[1]) || options.rectHeight}px;position:absolute;left:${coords[0] || 0}px;top:${coords[1] || 0}px;background-color:rgba(217, 197, 197, 0.5)" data-index="${index}">Area - ${count}</div>`);
                _this.append(oDiv);
                oDiv.draggable(options.draggableOptions);
                oDiv.resizable(options.resizableOptions);

                if(doingRecover==false){
                    oMap.append(`<area shape="rect" name="imagemaps-area" class="imagemaps-area${index}" coords="0,0,${options.rectWidth},${options.rectHeight}"/>`);
                }
                
                //WE WILL ADD DYNAMIC AREA COUNT TO THE SAVE BUTTON
                oTr.find(options.btnSave).attr('area-id', count);
                ...
                oTr.find(options.btnSave).unbind('click').on('click', function(){
